...
import {getName, saveName} from "../utils/session.js";

export default function Certificate() {
    const location = useLocation()

    const nameFromState = location.state?.name
    const name = nameFromState || getName()
    ...
    const pairName = useMemo(() => {
        if (!name) return "подруга"
        console.log("name", name)
        return name.trim().toLowerCase().includes("ксю") ?  "Юля" : "Ксюша"
    }, [name])
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            confetti({...defaults, particleCount: 80, spread: 110, startVelocity: 35})
        }, 250)
    }, [loading, name])

    const downloadCertificate = () => {
        const url = "/certificate.png" // лежить у public/
        const a = document.createElement("a")
        a.href = url
        a.download = "sobi-club-certificate.png"
                    ...
                    {loading ? (
                        ...
                        >
                            <div className="flex justify-center">
                                <div
                                    className="h-12 w-12 border-4 border-purple-200 border-t-purple-600 rounded-full animate-spin"/>
                            </div>
                        ...
                        >
                            ...
                            >
                                ...
                                <div className="mt-5 space-y-2 text-m text-gray-300">
                                    <p><span className="font-semibold">Де:</span> Sobi club</p>
                                    <p><span className="font-semibold">Коли:</span> 19.03.2026</p>
                                    <p><span className="font-semibold">З ким:</span> тільки ти і {pairName}</p>
                                </div>

                                {/* Preview (красиво показати саме сертифікат) */}
                                <div className="mt-5 overflow-hidden rounded-2xl border border-white/60 shadow">
                                    <img
                                        src="/certificate.png"
                                        alt="SPA Certificate"
